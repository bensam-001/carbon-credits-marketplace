module carbon_credit_marketplace::carbon_credit_marketplace {
    ...
    public entry fun buy_credit(credit: &mut CarbonCredit, amount: Coin<SUI>, ctx: &mut TxContext) {
        assert!(!is_some(&credit.buyer), EInvalidBid);
        assert!(clock::timestamp_ms(clock) <= credit.deadline, EExpiredCredit);
